...
$link = mysqli_connect("localhost", "root", "", "food_web");
                ...
                while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                    ...
                    <div class="col-md-3 card" style="margin-top:12px;">
                            <div style="overflow:hidden;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2); background-color:white;  padding:16px; height:400px;" align="center">
                            	<img height="200" width="200px" src="../assets/'.$row["file_path"].'" class="img-ressponsive" /><br />
                            	<h4 class="text-info">'.$row["food_name"].'</h4>
                            	<h4 class="text-danger">Ksh. '.$row["price"] .'</h4>
                            	<input type="text" name="quantity" id="quantity' . $row["fid"] .'" class="form-control" value="1" />
                            	...
                            	<input type="button" name="add_to_cart" id="'.$row["fid"].'" style="margin-top:5px;" class="btn btn-success form-control add_to_cart" value="Add to Cart" />
                            </div>
                        </div>
                		';
                }
